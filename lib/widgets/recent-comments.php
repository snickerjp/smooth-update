...
class RecentCommentsWidgetItem extends WP_Widget {
  function __construct() {
     parent::__construct(
      'recent_comments',
      __( '[S] 最近のコメント', 'smooth-update' ),//ウイジェット名
      array('description' => __( '最近投稿されたコメントを表示するウィジェットです。', 'smooth-update' ))
    );
  }
  ...
  function form($instance) {
    ...
    <p>
      <label for="<?php echo $this->get_field_id('count'); ?>">
        <?php _e( '表示するコメント数', 'smooth-update' ) ?>
      </label>
    ...
    <p>
      <label for="<?php echo $this->get_field_id('author_not_in'); ?>">
        <?php _e( '管理者の除外', 'smooth-update' ) ?>
      </label><br />
      <input class="widefat" id="<?php echo $this->get_field_id('author_not_in'); ?>" name="<?php echo $this->get_field_name('author_not_in'); ?>" type="checkbox" value="on"<?php echo ($author_not_in ? ' checked="checked"' : ''); ?> /><?php _e( '管理者のコメントを表示しない', 'smooth-update' ) ?>
    </p>
    <?php
  }
}
