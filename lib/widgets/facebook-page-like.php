...
class FacebookPageLikeWidgetItem extends WP_Widget {
  function __construct() {
    parent::__construct(
      'facebook_page_like',
      __( '[S] Facebookページ「いいね！」', 'smooth-update' ), //ウイジェット名
      array('description' => __( '投稿・個別ページのアイキャッチを利用したFacebookページへの「いいね！」ボタンを表示するSimplicity用ウィジェットです。（※「Facebookページ」のみで利用できます。「個人ページ」で設定されている場合は利用できません）', 'smooth-update' ))
    );
  }
  ...
  function form($instance) {
    ...
    <p>
      ...
      <?php _e( 'タイトル（未入力で非表示）', 'smooth-update' ) ?>
      </label>
    ...
    <p>
      <label for="<?php echo $this->get_field_id('text_facebook_page_like'); ?>">
        <?php _e( 'メッセージ', 'smooth-update' ) ?>
      <?php
        if ( !$text ) {
          $text = sprintf( __( 'この記事をお届けした<br>%sの最新ニュース情報を、<br><span class="bold-red">いいね</span>してチェックしよう！', 'smooth-update' ), get_bloginfo('name') );
        }?>
      </label>
