...
class SimplicityNewEntryWidgetItem extends WP_Widget {
  function __construct() {
    parent::__construct(
      'new_entries',
      __( '[S] 新着記事', 'smooth-update' ),
      array('description' => __( '新着記事リストを表示するSimplicityウィジェットです。', 'smooth-update' ))
    );//ウイジェット名
  }
      ...
      if ($title !== null) {
        ...
        if ($title) {
          ...
          if ( $widget_mode == 'all' ) {//全ての表示モードの時は
            ...
            echo __( 'カテゴリー別新着記事', 'smooth-update' );
          }
          //echo '新着記事';
        }
  ...
  function form($instance) {
    ...
    <p>
      ...
      <input class="widefat" id="<?php echo $this->get_field_id('widget_mode'); ?>" name="<?php echo $this->get_field_name('widget_mode'); ?>"  type="radio" value="all" <?php echo ( ($widget_mode == 'all' || !$widget_mode ) ? ' checked="checked"' : ""); ?> /><?php _e( '全ての新着記事（全ページで表示）', 'smooth-update' ) ?><br />
      <input class="widefat" id="<?php echo $this->get_field_id('widget_mode'); ?>" name="<?php echo $this->get_field_name('widget_mode'); ?>"  type="radio" value="category"<?php echo ($widget_mode == 'category' ? ' checked="checked"' : ""); ?> /><?php _e( 'カテゴリ別新着記事（投稿・カテゴリで表示）', 'smooth-update' ) ?><br />
    </p>
    <?php //タイトル入力フォーム ?>
    <p>
    ...
    <p>
      <label for="<?php echo $this->get_field_id('is_top_visible'); ?>">
        <?php _e( '全てのページで表示', 'smooth-update' ) ?>
      </label><br />
      <input class="widefat" id="<?php echo $this->get_field_id('is_top_visible'); ?>" name="<?php echo $this->get_field_name('is_top_visible'); ?>" type="checkbox" value="on"<?php echo ($is_top_visible ? ' checked="checked"' : ''); ?> /><?php _e( '表示する<br>※新着順のインデックスリストが表示されるトップでも表示する場合はチェックしてください。<br>※「表示モード」が「全ての新着記事」のときのみ有効な機能です。', 'smooth-update' ) ?>
    </p>
    <?php endif ?>
