...
class SimplicitySocialFollowWidgetItem extends WP_Widget {
  function __construct() {
    parent::__construct(
      'sns_follow_buttons',
      __( '[S] SNSフォローボタン', 'smooth-update' ),
      array('description' => __( 'SNSサービスのフォローアイコンボタンを表示するSimplicityウィジェットです。', 'smooth-update' ))
    );
  }
  function widget($args, $instance) {
    ...
    if ($title !== null) {
      ...
      if ($title) {
        ...
        echo __( 'SNSフォローボタン', 'smooth-update' );
      }
  ...
  function form($instance) {
    ...
    <p>
       <label for="<?php echo $this->get_field_id('title'); ?>">
        <?php _e( 'SNSフォローボタンのタイトル', 'smooth-update' ) ?>
       </label>
