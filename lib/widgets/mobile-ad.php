...
class MobileAdWidgetItem extends WP_Widget {
  function __construct() {
    parent::__construct(
      'mobile_ad',
      __( '[S] モバイル用広告', 'smooth-update' ), //ウイジェット名
      array('description' => __( 'モバイルのみで表示されるSimplicity用の広告ウィジェットです。（※アドセンスの場合は広告コードのみを記入してください。）', 'smooth-update' ))
    );
  }
...
<p>
  <label for="<?php echo $this->get_field_id('margin_left_px'); ?>">
    <?php _e( '左マージンのピクセル数（－指定で左に移動）', 'smooth-update' ) ?>
  </label>
...
<p>
  <label for="<?php echo $this->get_field_id('is_exclude_ads_enable'); ?>">
    <?php _e( '広告除外設定の適用', 'smooth-update' ) ?>
  </label><br />
  <input class="widefat" id="<?php echo $this->get_field_id('is_exclude_ads_enable'); ?>" name="<?php echo $this->get_field_name('is_exclude_ads_enable'); ?>" type="checkbox" value="on"<?php echo ($is_exclude_ads_enable ? ' checked="checked"' : ''); ?> /><?php _e( 'カスタマイザーの広告除外設定を適用する', 'smooth-update' ) ?>
</p>
<?php
  }
}
